...
'''
        for an overview of all the endpoints go to: http://127.0.0.1:8000/docs

        http://127.0.0.1:8000/books
'''
@router.get(path="/books", description="returns all the books in the list")
async def get_books():
    return books
...
'''
@router.get(path="/books/Fourth Wing", description="returns Fourth Wing")
...
'''
@router.get(path="/books/{book_title}",description="returns the specified book from {book_title}")
...
'''
@router.get(path="/books/search/", description="returns the book(s) that contain the same category OR author used in the search paramaters, ie {category} and {author}")
...
'''
@router.get(path="/books/search/{book_author}/", description="returns the book(s) that contain the same category AND author used in the search paramaters, ie {category} and {book_author}")
...
'''
@router.post(path="/books/create_book", description="creates a new book and adds this instance into the current list of books")
async def create_book(new_book=Body()):
    books.append(new_book)

'''
        {"title": "Iron Flame", "author": "Rebecca Yaros", "genre": "Action"}
'''
@router.put(path="/books/update_book", description="used to update a book to the adjusted values, note the title must be the same")
...
'''
        {"title": "Iron Flame"}
'''
@router.delete(path="/books/delete_book", description="used to delete a book, note the title must be the same")
